...
async function versionCheck() {
    log.out(
        info('mark2jek') + important(`v${require('./package.json').version}`)
    );
    process.exit(0);
}
...
async function parseFlags(isSet) {
    ...
    for (let i in process.argv) {
        ...
        if (
            process.argv[i].toLowerCase().startsWith('new=') ||
            process.argv[i].toLowerCase().startsWith('--new=')
        ) {
            Config.flags.outputName.value = extractNew(process.argv[i]);
            if (Config.flags.outputName.value === ' ')
                Config.flags.outputName.value = '';
            log.info(
                `outputName / new= ${coloredValue(
                    Config.flags.outputName.value
                )}`
            );
        } else {
    ...
    function extractNew(arg) {
        return (arg.match(/new='([^']+)'/i) ||
            arg.match(/new=((?!')[^\s]+)/i) || [undefined, ''])[1];
    }
}
...
function doRegex(data) {
    ...
    if (Config.flags.pics?.value) {
        data = data.replace(/!\[.*\]\((https?:\/\/.*)\)/gi, '<img src="$1">');
    }
    if (Config.flags.raw?.value) {
        data = data.replace(
            /(https?:\/\/github.com\/[^\/]+\/[^\/]+\/blob\/.+\.(?:png|svg|jpg|jpeg|gif|webp|bmp)(?!\?raw=true))/gi,
            '$1?raw=true'
        );
    }
...
async function setup() {
    ...
    log.info(
        `for more info on each command, use ${code('m2jek help')} or ${code(
            'm2jek list'
        )}\n`
    );
    ...
    for (const [key, element] of Object.entries(Config.default)) {
        ...
        await prompt.question(
            ` ${important(key)} / ${important(element.alias)} (${coloredValue(
                savedValue
            )}):`,
            key
        );
        if (Config.flags[key].value !== savedValue) {
            log.info(
                `Saved new value: ${coloredValue(Config.flags[key].value)}`
            );
            changed ||= true;
        }
    }
...
async function list() {
    function printCommand(names, description, ...examples) {
        ...
        log.out(
            lines(namesString, info('> ' + description + examplesString), '')
        );
    }
    ...
    printCommand(
        ['filePath'],
        'run program on *filePath* using saved config or specified flags',
        {
            code: 'm2jek docs/readme.md',
            info: 'will modify and overwrite docs/readme.md'
        },
        {
            code: 'm2jek todo.txt --noRaw new=index.md',
            info: 'run without raw flag, saves to index.md'
        }
    );
    ...
    printCommand(
        ['new=filename.md', 'new="my long filename.md"'],
        'saves output to *filename*',
        {
            code: 'm2jek set --new="my web page.md"',
            info: 'always output to "my web page.md"'
        },
        {
            code: 'm2jek docs/input.txt --new=mypage.md',
            info: 'process input.txt and output to docs/mypage.md'
        }
    );
    ...
    printCommand(
        ['set', 'change'],
        `Change flags by keywords (in config)`,
        {
            code: 'm2jek set --nocodeblock expand',
            info: `codeblock ${red('off')}, expand ${green('on')}`
        },
...
async function flags() {
    ...
    log.info(
        '> if flagname = raw then noRaw is the disable flag (ignore case, valid for aliases too)'
    );
    log.info(
        '> if not writing flagname in `set` mode -> flagname needs to start with `--`.'
    );
    log.important('Examples:');
    log.out(
        `${code('m2jek input.md --noNested --pics')} ${info(
            'to overwrite input.md with no nest check and transforming image links'
        )}`
    );
    log.out(
        `${code('m2jek set noNested pics')} ${info(
            'to always use those those settings'
        )}\n`
    );
    ...
    for (const [key, element] of Object.entries(Config.default)) {
        log.out(
            ` ${important(key)} or ${important(
                element.alias
            )}   (${coloredValue(element.value)})`
        );
